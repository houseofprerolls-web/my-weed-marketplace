...
import { Users, Store, DollarSign, TrendingUp, CircleAlert as AlertCircle, CircleCheck as CheckCircle, Clock, Eye, Search, MapPin, Star, Flag, FileCheck, Circle as XCircle, ChartBar as BarChart3, Loader2 } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const [pendingVendors, setPendingVendors] = useState<PendingVendorRow[]>([]);
  const [pendingLoading, setPendingLoading] = useState(false);

  useEffect(() => {
    if (!isAdmin || !vendorsSchema) return;
    let cancelled = false;
    setPendingLoading(true);
    void (async () => {
      const { data, error } = await supabase
        .from('vendors')
        .select('id,name,city,state,zip,license_status,created_at')
        .in('license_status', ['pending', 'needs_review'])
        .order('created_at', { ascending: false })
        .limit(20);
      if (!cancelled) {
        setPendingLoading(false);
        if (!error && data) setPendingVendors(data as PendingVendorRow[]);
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-400 mt-1">
                      {!vendorsSchema
                        ? 'Enable NEXT_PUBLIC_USE_VENDORS_TABLE=1 to load pending applications from the database.'
                        : pendingLoading
                          ? 'Loading pending vendors…'
                          : `${pendingVendors.length} vendor(s) awaiting license review`}
                    </p>
                  </div>
                  <Badge className="bg-yellow-600/20 text-yellow-500 border-yellow-600/30">
                    {vendorsSchema ? pendingVendors.length : '—'} Pending
                  </Badge>
                </div>
              </div>
              <div className="p-6">
                {!vendorsSchema ? (
                  <p className="text-sm text-gray-500">
                    Vendor applications from{' '}
                    <Link href="/vendor/onboarding" className="text-green-400 hover:underline">
                      onboarding
                    </Link>{' '}
                    are stored on the <code className="text-gray-400">vendors</code> table when vendors schema is enabled.
                  </p>
                ) : pendingLoading ? (
                  ...
                  <p className="text-sm text-gray-500">No pending or needs_review vendors right now.</p>
                ) : (
                  <div className="space-y-4">
                    {pendingVendors.map((vendor) => {
                      const loc = [vendor.city, vendor.state].filter(Boolean).join(', ');
                      const zip = vendor.zip ? ` ${vendor.zip}` : '';
                      return (
                        <div key={vendor.id} className="rounded-lg border border-green-900/20 bg-gray-800/50 p-4">
                          <div className="mb-3 flex items-start justify-between">
                            <div>
                              <h4 className="text-lg font-bold text-white">{vendor.name}</h4>
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-green-900/20 pt-3">
                            <div className="flex items-center gap-2">
                              <FileCheck className="h-4 w-4 text-yellow-500" />
                              <span className="text-sm text-gray-300">License: {vendor.license_status}</span>
                            </div>
                            <Button size="sm" variant="outline" className="border-green-900/20 text-white hover:bg-green-500/10" asChild>
                              <Link href={`/admin/vendors?vendor=${encodeURIComponent(vendor.id)}`}>Review in vendor console</Link>
                            </Button>
                          </div>
                        </div>
                      );
                ...
                {vendorsSchema && !pendingLoading && pendingVendors.length > 0 ? (
                  <div className="mt-4 border-t border-green-900/20 pt-4">
                    <Button variant="outline" className="border-green-800 text-white" asChild>
                      <Link href="/admin/vendors">Open full vendor management</Link>
                    </Button>
                  </div>
                ) : null}
              </div>
