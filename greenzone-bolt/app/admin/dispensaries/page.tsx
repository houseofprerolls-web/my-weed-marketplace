    ...
    <div className="basis-full space-y-3 border-t border-gray-800 pt-3">
      <div className="flex flex-wrap gap-2">
        <Input
          ...
          className="max-w-xs border-gray-700 bg-background"
        />
      ...
      <div className="flex flex-wrap items-end gap-2">
        <div className="space-y-1">
          ...
          <Input
            ...
            placeholder="uuid"
            className="w-72 border-gray-700 bg-background font-mono text-xs"
          />
        </div>
...
export default function AdminDispensariesPage() {
  ...
  const [reverseUserSearch, setReverseUserSearch] = useState('');
  const [reverseProfileHits, setReverseProfileHits] = useState<ProfileHit[]>([]);
  const [reverseVendorSearch, setReverseVendorSearch] = useState('');
  const [reverseSelectedUserId, setReverseSelectedUserId] = useState<string | null>(null);
  ...
  const reverseVendorFiltered = useMemo(() => {
    const q = reverseVendorSearch.trim().toLowerCase();
    if (!q) return allDispensaries;
    return allDispensaries.filter((v) => {
      const blob = [v.name, v.slug, v.zip, v.phone].filter(Boolean).join(' ').toLowerCase();
      return blob.includes(q);
    });
  }, [allDispensaries, reverseVendorSearch]);
  ...
  const searchProfilesReverse = useCallback(async () => {
    const q = reverseUserSearch.trim();
    if (q.length < 2) {
      setReverseProfileHits([]);
      return;
    }
    const { data, error } = await supabase
      .from('profiles')
    ...
    setReverseProfileHits((data || []) as ProfileHit[]);
  }, [reverseUserSearch, toast]);
  ...
  async function handleReverseLink(vendorId: string) {
    if (!reverseSelectedUserId) {
      toast({ title: 'Pick a user first', description: 'Search and select a user to link.', variant: 'destructive' });
      return;
    }
    await handleLink(vendorId, reverseSelectedUserId);
  }
      ...
      <div className="container mx-auto max-w-6xl space-y-10 px-4 py-10">
        <Card className="border-green-900/30 bg-gradient-to-br from-gray-900 to-black p-6">
          ...
          <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} required className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2">
              <Label>ZIP *</Label>
              <Input value={zip} onChange={(e) => setZip(e.target.value)} required className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2">
              <Label>Phone</Label>
              <Input value={phone} onChange={(e) => setPhone(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2">
              <Label>City</Label>
              <Input value={city} onChange={(e) => setCity(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2">
              <Label>State</Label>
              <Input value={stateVal} onChange={(e) => setStateVal(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label>Cannabis license (LIC)</Label>
              <Input value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label>Description</Label>
              <Input value={description} onChange={(e) => setDescription(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label>Slug (optional — auto if empty)</Label>
              <Input value={slug} onChange={(e) => setSlug(e.target.value)} className="border-gray-700 bg-background" />
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label>Internal contact note (optional)</Label>
              <Input value={contactNote} onChange={(e) => setContactNote(e.target.value)} className="border-gray-700 bg-background" />
            </div>
        ...
        <Card className="border-green-900/30 bg-gradient-to-br from-gray-900 to-black p-6">
          <h2 className="mb-2 flex items-center gap-2 text-xl font-semibold">
            <UserCog className="h-5 w-5 text-brand-lime" />
            Link from user to dispensary
          </h2>
          <p className="mb-4 text-sm text-gray-400">
            Search a user first, then search dispensaries and link in one click.
          </p>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3 rounded-lg border border-gray-800 bg-black/30 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Input
                  placeholder="Search email or name…"
                  value={reverseUserSearch}
                  onChange={(e) => setReverseUserSearch(e.target.value)}
                  className="max-w-xs border-gray-700 bg-background"
                />
                <Button type="button" variant="secondary" onClick={searchProfilesReverse}>
                  <Search className="mr-1 h-4 w-4" />
                  Search
                </Button>
              </div>
              {reverseProfileHits.length > 0 ? (
                <ul className="space-y-1 text-sm">
                  {reverseProfileHits.map((p) => {
                    const active = reverseSelectedUserId === p.id;
                    return (
                      <li
                        key={p.id}
                        className="flex items-center justify-between gap-2 rounded border border-gray-800 px-2 py-1"
                      >
                        <span className="truncate text-gray-300">
                          {p.email ?? p.id} {p.full_name ? `· ${p.full_name}` : ''}
                        </span>
                        <Button
                          ...
                          onClick={() => setReverseSelectedUserId(p.id)}
                        >
            ...
            <div className="space-y-3 rounded-lg border border-gray-800 bg-black/30 p-4">
              <Label>Dispensary search</Label>
              <Input
                placeholder="Search dispensary, slug, ZIP, phone…"
                value={reverseVendorSearch}
                onChange={(e) => setReverseVendorSearch(e.target.value)}
                className="max-w-md border-gray-700 bg-background"
              />
              <div className="max-h-72 overflow-y-auto overflow-x-hidden rounded border border-gray-800">
                <ul className="divide-y divide-gray-800">
                  {reverseVendorFiltered.slice(0, 40).map((v) => (
                    <li key={v.id} className="flex min-w-0 items-center justify-between gap-2 px-3 py-2 text-sm">
                      <span className="min-w-0 flex-1 truncate text-gray-300">
                        {v.name} · {v.slug} · ZIP {v.zip ?? '—'}
                      </span>
                      <Button
                        type="button"
                        size="sm"
                        className="relative z-[1] shrink-0"
                        disabled={linking || !reverseSelectedUserId}
                        onClick={() => handleReverseLink(v.id)}
                      >
                        Link
                      </Button>
                    </li>
              ...
              <p className="text-xs text-gray-500">Showing up to 40 matches. Select a user first to enable link buttons.</p>
            </div>
          </div>
        </Card>
          ...
          {unclaimed.length === 0 ? (
            ...
            <ul className="space-y-3">
              {unclaimed.map((v) => (
                <li
                  key={v.id}
                  className="flex min-w-0 flex-col gap-3 rounded-lg border border-gray-800 bg-black/40 p-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between"
                >
                  ...
                  <div className="relative z-[1] flex shrink-0 flex-wrap gap-2">
                    <Button asChild size="sm" variant="outline" className="border-gray-600">
                      <Link href={`/admin/vendors?vendor=${encodeURIComponent(v.id)}`}>Edit in vendor admin</Link>
                    </Button>
                    <Button
                      size="sm"
        ...
        <Card className="border-green-900/30 bg-gradient-to-br from-gray-900 to-black p-6">
          ...
          <div className="max-h-[min(70vh,720px)] overflow-y-auto overflow-x-hidden rounded-lg border border-gray-800">
            <ul className="divide-y divide-gray-800">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative z-[1] flex shrink-0 flex-wrap items-center gap-2">
                      ...
                      <Button asChild size="sm" variant="outline" className="border-gray-600">
                        <Link href={`/admin/vendors?vendor=${encodeURIComponent(v.id)}`}>Vendor admin</Link>
                      </Button>
                      {claimed && (
                        <Button asChild size="sm" variant="outline" className="border-green-600/30 text-green-400 hover:bg-green-600/10">
                          <Link href={listingHrefForVendor({ id: v.id, slug: v.slug })}>
