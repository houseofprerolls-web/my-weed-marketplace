...
export default function VendorSupplyRfqPage() {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <Button asChild variant="ghost" size="sm" className="-ml-2 text-zinc-400 hover:text-white">
        <Link
          href={q(
            supplierSlug ? `/vendor/supply/${encodeURIComponent(supplierSlug)}` : '/vendor/supply/directory'
          )}
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">
          {loadingMeta ? 'Loading…' : supplierName || 'Supplier'}
        </p>
      </div>
      {vLoading ? (
