...
Get-Content .env | ForEach-Object {
  ...
  if ($t -match '^NEXT_PUBLIC_USE_VENDORS_TABLE=(.+)$') { $pubVendors = $matches[1].Trim().Trim('"') }
  if ($t -match '^USE_VENDORS_TABLE=(.+)$') { $srvVendors = $matches[1].Trim().Trim('"') }
}
...
if (-not $pubVendors) { $pubVendors = '1' }
if (-not $srvVendors) { $srvVendors = '1' }
...
Write-Host "NEXT_PUBLIC_USE_VENDORS_TABLE (production)..."
npx vercel env add NEXT_PUBLIC_USE_VENDORS_TABLE production --value $pubVendors --yes --force
Write-Host "NEXT_PUBLIC_USE_VENDORS_TABLE (development)..."
npx vercel env add NEXT_PUBLIC_USE_VENDORS_TABLE development --value $pubVendors --yes --force

Write-Host "USE_VENDORS_TABLE (production)..."
npx vercel env add USE_VENDORS_TABLE production --value $srvVendors --yes --force
Write-Host "USE_VENDORS_TABLE (development)..."
npx vercel env add USE_VENDORS_TABLE development --value $srvVendors --yes --force
