...
export type VendorsCartLine = {
  kind: 'vendors';
  productId: string;
  /** When set, same SKU can appear as separate lines per quantity tier. */
  tierId?: string | null;
  /** Display only (e.g. "7 g"); optional for legacy carts. */
  ...
  thc: number | null;
  cbd: number | null;
  /** Shelf brand line (linked brand, custom label, or store name). */
  brandLabel?: string | null;
};

export function vendorsCartLineKey(productId: string, tierId?: string | null): string {
  const t = tierId?.trim() || '';
  return `${productId}\u0001${t}`;
}
...
function migrateRawToItems(raw: unknown): CartItem[] {
  ...
  for (const x of raw) {
    ...
    if (typeof o.vendorId === 'string') {
      const tid = typeof o.tierId === 'string' && o.tierId.trim() ? o.tierId.trim() : null;
      out.push({
        kind: 'vendors',
        productId: String(o.productId ?? ''),
        tierId: tid,
        quantityLabel: typeof o.quantityLabel === 'string' && o.quantityLabel.trim() ? o.quantityLabel.trim() : null,
        vendorId: o.vendorId,
...
function tryAddVendorsLine(
  ...
  const key = vendorsCartLineKey(line.productId, line.tierId);
  const existing = prev.find(
    (i) =>
      i.kind === 'vendors' && vendorsCartLineKey(i.productId, i.tierId) === key
  );
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeItem = useCallback((productId: string, tierId?: string | null) => {
    const key = vendorsCartLineKey(productId, tierId);
    const next = itemsRef.current.filter((i) => {
      if (i.kind === 'vendors') {
        return vendorsCartLineKey(i.productId, i.tierId) !== key;
      }
  ...
  const updateQuantity = useCallback((productId: string, quantity: number, tierId?: string | null) => {
    const key = vendorsCartLineKey(productId, tierId);
    if (quantity <= 0) {
      const next = itemsRef.current.filter((i) => {
        if (i.kind === 'vendors') {
    ...
    const next = itemsRef.current.map((i) => {
      if (i.kind === 'vendors') {
        return vendorsCartLineKey(i.productId, i.tierId) === key ? { ...i, quantity } : i;
      }
