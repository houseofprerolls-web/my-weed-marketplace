...
export type VendorMapAreaGate = {
  ...
  enabledMarkets: { id: string; name: string; slug: string | null }[];
};
...
export async function loadVendorMapAreaGate(vendorId: string): Promise<LoadVendorMapAreaGateResult> {
  try {
    ...
    let enabledMarkets: { id: string; name: string; slug: string | null }[] = [];
    if (approvedMarketIds.size > 0) {
      ...
      enabledMarkets = (names || []) as { id: string; name: string; slug: string | null }[];
      const byId = new Map(enabledMarkets.map((n) => [n.id, n.name]));
