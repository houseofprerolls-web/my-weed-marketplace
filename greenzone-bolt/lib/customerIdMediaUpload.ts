...
export async function uploadCustomerIdDocumentFile(
  ...
  // `vendor-media` bucket RLS expects the *first* path segment to match `auth.uid()`.
  // Store customer documents under `{userId}/id/...` so uploads are allowed.
  const path = `${userId}/id/${documentType}/${Date.now()}-${sanitizeFileName(file.name)}`;
