...
function parseLine(raw: unknown): ReorderResolveLine | null {
  ...
  const tier_id = typeof o.tier_id === 'string' && o.tier_id.trim() ? o.tier_id.trim() : null;
  const quantity_label =
    typeof o.quantity_label === 'string' && o.quantity_label.trim() ? o.quantity_label.trim() : null;
...
export async function runCustomerReorderFlow(
  ...
  return { ok: true, path: listingHrefForVendor({ id: res.vendor_id }) };
}
...
export function applyReorderLinesToCart(payload: ReorderResolveOk, cart: CartOps): boolean {
  ...
  const base = (ln: ReorderResolveLine): Omit<VendorsCartLine, 'kind'> => ({
    ...
    tierId: ln.tier_id ?? null,
    quantityLabel: ln.quantity_label ?? null,
    name: ln.name,
