...
export function useVendorAnalyticsScope(
  ...
  // If the saved scope no longer matches any linked shop, Radix Select shows a blank label.
  useEffect(() => {
    if (ownedVendors.length <= 1) return;
    const ok = scope === 'all' || ownedVendors.some((v) => v.id === scope);
    if (!ok) setScope('all');
  }, [ownedVendors, scope]);

  const effectiveVendorIds = useMemo(() => {
    // While `ownedVendors` is still filling in (or RLS returns rows only on `vendor`), fall back to the active shop id
    // so menu / dashboard queries do not run with an empty vendor_id filter.
    if (ownedVendors.length === 0) return vendor?.id ? [vendor.id] : [];
    if (scope === 'all') return ownedVendors.map((v) => v.id);
    return ownedVendors.some((v) => v.id === scope) ? [scope] : ownedVendors.map((v) => v.id);
  }, [scope, ownedVendors, vendor?.id]);
