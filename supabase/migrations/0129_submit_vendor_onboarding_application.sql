-- Self-service vendor onboarding: create/update pending vendor + business_licenses for admin review.
-- business_licenses writes are admin-only (0087); applicants use this SECURITY DEFINER RPC.
...
revoke all on function public.submit_vendor_onboarding_application(
  text, text, text, text, text, text,
  text, text, text, text,
  text, text, date, date
) from public;
grant execute on function public.submit_vendor_onboarding_application(
  text, text, text, text, text, text,
  text, text, text, text,
  text, text, date, date
) to authenticated;

comment on function public.submit_vendor_onboarding_application(
  text, text, text, text, text, text,
  text, text, text, text,
  text, text, date, date
) is
  'Applicant submits onboarding; upserts pending vendor + pending business_licenses row for admin review in Vendor management.';
