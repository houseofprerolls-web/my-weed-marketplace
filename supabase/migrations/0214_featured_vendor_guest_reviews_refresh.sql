-- Refresh guest seeded 5★ vendor reviews for four spotlight shops only.
-- Safe after 0213 or on its own: does NOT delete other reviews or reset catalog/vendor globals.
-- Removes prior guest rows (null reviewer + display handle) for each matched vendor, then re-inserts.
...
begin
  ...
  if vid is not null then
    touched := array_append(touched, vid);
    delete from public.reviews r
    where r.entity_type = 'vendor'
      and r.entity_id = vid
      and r.reviewer_id is null
      and (
        r.reviewer_display_handle is not null
        or (r.body is null and r.title = 'Rating')
      );
    cnt := 58;
    for n in 1..cnt loop
      created_ts :=
  ...
  if coalesce(array_length(touched, 1), 0) < 1 then
    return;
  end if;
  ...
  and exists (
    ...
    set
      total_reviews = s.c,
      average_rating = round(coalesce(s.avg_r, 0)::numeric, 2)
    from (
      select
        entity_id as vendor_id,
        count(*)::int as c,
        avg(r.rating::numeric) as avg_r
      from public.reviews r
      where r.entity_type = 'vendor'
        and r.entity_id = any (touched)
      group by entity_id
    ) s
