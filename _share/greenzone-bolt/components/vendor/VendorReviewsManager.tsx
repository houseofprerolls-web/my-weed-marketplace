...
export function VendorReviewsManager({ vendorId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('reviews')
      .select('id, reviewer_id, rating, title, body, created_at, vendor_reply, vendor_reply_at')
    ...
    const list = (data || []) as Omit<ReviewManageRow, 'reviewer_name'>[];
    ...
    if (ids.length) {
      ...
      for (const p of profs || []) {
        ...
        nameById.set(row.id, (row.full_name || '').trim() || 'Customer');
      }
    }
    ...
    setRows(
      list.map((r) => ({
        ...r,
        reviewer_name: r.reviewer_id && nameById.has(r.reviewer_id) ? nameById.get(r.reviewer_id)! : 'Customer',
      }))
    );
