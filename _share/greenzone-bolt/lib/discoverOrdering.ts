...
/** Slot rank 1 = strongest weight; rank 10 = weakest but still eligible (never 0 weight). */
export function weightForTreehouseSlotRank(rank: number): number {
  const r = Math.max(1, Math.min(10, Math.round(rank)));
  const w = (11 - r) * (11 - r);
  return Math.max(1, w);
}
...
function sortTailByDistance(rest: DiscoveryVendor[], ctx: DiscoverSortContext): void {
  ...
  const distKm = (v: DiscoveryVendor) => {
    if (!geoOk || v.geo_lat == null || v.geo_lng == null) return Number.POSITIVE_INFINITY;
    return haversineKm(ctx.shopperLat!, ctx.shopperLng!, v.geo_lat, v.geo_lng);
  };
  rest.sort((a, b) => {
    const da = distKm(a);
    const db = distKm(b);
...
export function sortDiscoverTail(
  ...
  switch (sortBy) {
    ...
    case 'highest-rated':
      rest.sort(
        (a, b) =>
          b.average_rating - a.average_rating || a.business_name.localeCompare(b.business_name)
      );
      break;
    case 'most-popular':
      rest.sort(
        (a, b) =>
          b.total_reviews - a.total_reviews || a.business_name.localeCompare(b.business_name)
      );
      break;
    case 'newest':
      rest.sort(
...
/**
 * `pinned` is usually up to two Smokers Club tree placements (slot 1–10), chosen once per market/load.
